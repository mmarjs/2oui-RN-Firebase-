...
export default class DatePicker extends PureComponent {
  ...
  static defaultProps = {
    ...
    order: 'D-M-Y',
    mode: 'date',
    maximumDate: moment().add(40, 'years').toDate(),
    minimumDate: moment().add(-40, 'years').toDate(),
    date: moment().add(-20,'years').toDate(),
    style: null,
    textColor: '#333',
    textSize: 26,
    itemSpace: 20,
  ...
  constructor(props) {
    ...
    for (let i = 1; i <= 12; i += 1) {
      this.state.monthRange.push({ value: i, label: `${monthName[i-1]}${labelUnit.month}` });
    }
